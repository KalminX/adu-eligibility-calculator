...
class NotificationServicesTestCase(TestCase):
    ...
    @override_settings(RESEND_API_KEY="test-resend-key", DEFAULT_FROM_EMAIL="test@example.com", LEAD_NOTIFICATION_EMAIL="lead@example.com")
    @patch("requests.post")
    def test_email_live_path_success(self, mock_post):
    ...
    @patch("requests.post")
    def test_email_live_path_handles_api_failure(self, mock_post):
        mock_response = MagicMock()
